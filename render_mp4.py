...
def render_continuous_video(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform):
    """连续时间渲染，生成视频"""
    render_path = os.path.join(model_path, name, "continuous_{}".format(iteration), "renders")
    depth_path = os.path.join(model_path, name, "continuous_{}".format(iteration), "depth")
    ...
    # 创建相机路径
    angles = np.linspace(-180, 180, frame + 1)[:-1]
    poses = torch.stack([pose_spherical(angle, -30.0, 4.0) for angle in angles], 0)
    
    for t in tqdm(range(frame), desc="Rendering continuous video"):
        # 更新相机位姿
        pose = poses[t]
        matrix = np.linalg.inv(np.array(pose))
        R = -np.transpose(matrix[:3, :3])
    ...
    renderings = np.stack(renderings, 0).transpose(0, 2, 3, 1)
    imageio.mimwrite(os.path.join(render_path, 'video.mp4'), renderings, fps=30, quality=8)
    print(f"Video saved to {os.path.join(render_path, 'video.mp4')}")


def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
                mode: str):
    """
    Renders train/test sets from model
    """
    with torch.no_grad():
        ...
        elif mode == "continuous":  # 新添加的模式
            render_func = render_continuous_video
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", default='render', choices=['render', 'time', 'view', 'all', 'pose', 'original', 'continuous'])
